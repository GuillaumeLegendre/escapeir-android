 ...
 * a square path in the opposite direction of the hand of a clock. This move never stop itself
...
public class SquareLeft implements Movable{
	...
	public void move(Body body) {
		...
		case 0:
			...
			break;
		case 1:
			if(body.getPosition().x*EscapeWorld.SCALE>(Game.getTheGame().getWidth()/3*2)){
				return;
			}
			v2.set(0f,2f);
			break;
		case 2:
			...
			break;
		case 3:
			if(body.getPosition().x*EscapeWorld.SCALE<(Game.getTheGame().getWidth()/3*2+120)){
				return;
			}
			v2.set(0f,-2f);
			break;
